...
function showContent(showId, hideId) {
    document.getElementById(hideId).classList.add('hide-on-select');
    document.getElementById(showId).classList.remove('hide-on-select');
}
...
function handleFormSubmit() {
    ...
    if (loggedInRole && loggedInRole !== role) {
        alert('Ya has iniciado sesión como ' + loggedInRole + '. Debes cerrar sesión antes de cambiar de rol.');
        return false;
    }

    loggedInRole = role;

    // Ocultar el enlace no seleccionado y mostrar el enlace seleccionado después de iniciar sesión
    const adminLink = document.getElementById('adminLink');
    const tecnicoLink = document.getElementById('tecnicoLink');

    if (adminLink && tecnicoLink) {
        if (loggedInRole === 'Administrador') {
            showContent('adminLink', 'tecnicoLink');
        } else if (loggedInRole === 'Tecnico') {
            showContent('tecnicoLink', 'adminLink');
        }
    }
...
// Verificar el rol actual después de cargar la página y mostrar/ocultar según sea necesario
document.addEventListener('DOMContentLoaded', function () {
    const roleRadios = document.querySelectorAll('input[name="role"]');
    const selectedRole = Array.from(roleRadios).find(radio => radio.checked);
